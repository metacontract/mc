...
library Validator {
        ...
        return (
            validate(COMPLETION, func.name.isAssigned(), HEAD.FUNC_NAME_UNASSIGNED, BODY.FUNC_NAME_UNASSIGNED)
                && validate(
                    ...
                    HEAD.FUNC_CONTRACT_UNASSIGNED,
                    BODY.FUNC_CONTRACT_UNASSIGNED
                )
        );
    }
    ...
    function ValidateBuilder(Bundle storage bundle) internal view returns (bool) {
        return (
            validate(COMPLETION, bundle.name.isNotEmpty(), HEAD.BUNDLE_NAME_UNASSIGNED, BODY.BUNDLE_NAME_UNASSIGNED)
                && validate(
                    COMPLETION,
                    bundle.functions.length.isNotZero(),
                    HEAD.NO_FUNCTIONS_IN_BUNDLE,
                    BODY.NO_FUNCTIONS_IN_BUNDLE
                )
                && validate(
                    COMPLETION, bundle.facade.isContract(), HEAD.BUNDLE_FACADE_UNASSIGNED, BODY.BUNDLE_FACADE_UNASSIGNED
                )
        );
    }
    ...
    function MUST_HaveFunction(Bundle storage bundle) internal view {
        validate(
            MUST, bundle.functions.length.isNotZero(), HEAD.NO_FUNCTIONS_IN_BUNDLE, BODY.NO_FUNCTIONS_IN_BUNDLE_REQUIRED
        );
    }
    ...
    function MUST_HaveUniqueSelector(Bundle storage bundle, Function storage func) internal view {
        validate(
            MUST,
            bundle.hasNotSameSelector(func),
            HEAD.BUNDLE_CONTAINS_SAME_SELECTOR,
            BODY.BUNDLE_CONTAINS_SAME_SELECTOR
        );
    }
    ...
    function MUST_ExistCurrentName(BundleRegistry storage registry) internal view {
        validate(
            MUST,
            registry.current.name.isAssigned(),
            HEAD.CURRENT_BUNDLE_NOT_EXIST,
            BODY.CURRENT_BUNDLE_NOT_EXIST_REQUIRED
        );
    }
    ...
    function MUST_NotRegistered(ProxyRegistry storage registry, string memory name) internal view {
        validate(
            MUST, registry.proxies[name].isUninitialized(), HEAD.PROXY_ALREADY_REGISTERED, BODY.PROXY_ALREADY_REGISTERED
        );
    }
    ...
    function ValidateBuilder(Dictionary memory dictionary) internal view returns (bool) {
        return (
            validate(
                ...
                HEAD.DICTIONARY_NAME_UNASSIGNED,
                BODY.DICTIONARY_NAME_UNASSIGNED
            )
                && validate(
                    COMPLETION,
                    dictionary.addr.isContract(),
                    HEAD.DICTIONARY_ADDR_UNASSIGNED,
                    BODY.DICTIONARY_ADDR_UNASSIGNED
                )
                && validate(
                    COMPLETION,
                    dictionary.kind.isNotUndefined(),
                    HEAD.DICTIONARY_KIND_UNDEFINED,
                    BODY.DICTIONARY_KIND_UNDEFINED
                )
        );
    }
    ...
    function MUST_Registered(DictionaryRegistry storage registry, string memory name) internal view {
        validate(
            MUST,
            registry.dictionaries[name].isComplete(),
            HEAD.DICTIONARY_NOT_REGISTERED,
            BODY.DICTIONARY_NOT_REGISTERED
        );
    }
    ...
    function MUST_NotRegistered(DictionaryRegistry storage registry, string memory name) internal view {
        validate(
            MUST,
            registry.dictionaries[name].isUninitialized(),
            HEAD.DICTIONARY_ALREADY_REGISTERED,
            BODY.DICTIONARY_ALREADY_REGISTERED
        );
    }
    ...
    function MUST_ExistCurrentName(DictionaryRegistry storage registry) internal view {
        validate(
            MUST,
            registry.current.name.isAssigned(),
            HEAD.CURRENT_DICTIONARY_NOT_EXIST,
            BODY.CURRENT_DICTIONARY_NOT_EXIST
        );
    }
    ...
    function ValidateBuilder(StdRegistry storage registry) internal view returns (bool) {
        return (Validator.ValidateBuilder(registry.functions) && Validator.ValidateBuilder(registry.all));
    }
    ...
    function ValidateBuilder(StdFunctions storage std) internal view returns (bool) {
        return (
            Validator.ValidateBuilder(std.initSetAdmin) && Validator.ValidateBuilder(std.getFunctions)
