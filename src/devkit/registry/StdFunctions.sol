...
library StdFunctionsLib {
    ...
    function complete(StdFunctions storage std) internal returns (StdFunctions storage) {
        uint256 pid = std.startProcess("complete");
        std.fetch();
    ...
    function fetch(StdFunctions storage std) internal returns (StdFunctions storage) {
        uint256 pid = std.startProcess("fetch");
        std.startBuilding();
        std.fetch_InitSetAdmin().fetch_GetFunctions().fetch_Clone();
        std.finishBuilding();
        return std.finishProcess(pid);
    }
    ...
    function fetch_InitSetAdmin(StdFunctions storage std) internal returns (StdFunctions storage) {
        uint256 pid = std.startProcess("fetch_InitSetAdmin");
        std.initSetAdmin.fetch("InitSetAdmin").assignSelector(InitSetAdmin.initSetAdmin.selector);
    ...
    function fetch_GetFunctions(StdFunctions storage std) internal returns (StdFunctions storage) {
        uint256 pid = std.startProcess("fetch_GetFunctions");
        std.getFunctions.fetch("GetFunctions").assignSelector(GetFunctions.getFunctions.selector);
    ...
    function fetch_Clone(StdFunctions storage std) internal returns (StdFunctions storage) {
        uint256 pid = std.startProcess("fetch_Clone");
        std.clone.fetch("Clone").assignSelector(Clone.clone.selector);
    ...
    function deployIfNotExists(StdFunctions storage std) internal returns (StdFunctions storage) {
        uint256 pid = std.startProcess("deployIfNotExists");
        std.startBuilding();
        std.deployIfNotExists_InitSetAdmin().deployIfNotExists_GetFunctions().deployIfNotExists_Clone();
        std.finishBuilding();
        return std.finishProcess(pid);
    }
    /**
    ...
    function deployIfNotExists_InitSetAdmin(StdFunctions storage std) internal returns (StdFunctions storage) {
        uint256 pid = std.startProcess("deployIfNotExists_InitSetAdmin");
    ...
    function deployIfNotExists_GetFunctions(StdFunctions storage std) internal returns (StdFunctions storage) {
        uint256 pid = std.startProcess("deployIfNotExists_GetFunctions");
    ...
    function deployIfNotExists_Clone(StdFunctions storage std) internal returns (StdFunctions storage) {
        uint256 pid = std.startProcess("deployIfNotExists_Clone");
