...
library StdRegistryLib {
    ...
    function complete(StdRegistry storage registry) internal returns (StdRegistry storage) {
        uint256 pid = registry.startProcess("complete");
    ...
    function configureStdBundle(StdRegistry storage registry) internal returns (StdRegistry storage) {
        uint256 pid = registry.startProcess("configureStdBundle");
    ...
    function configureStdBundle_All(StdRegistry storage registry) internal returns (StdRegistry storage) {
        uint256 pid = registry.startProcess("configureStdBundle_All");
        registry.all.assignName("ALL_STD_FUNCTIONS").pushFunction(registry.functions.initSetAdmin).pushFunction(
            registry.functions.getFunctions
        ).pushFunction(registry.functions.clone).assignFacade(address(new StdFacade()));
        return registry.finishProcess(pid);
    }
}
