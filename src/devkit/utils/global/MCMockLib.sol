...
library MCMockLib {
    ...
    {
        uint256 pid = mc.startProcess("createMockProxy", param(bundle, initData));
    ...
    function createMockProxy(MCDevKit storage mc, Bundle storage bundle) internal returns (Proxy storage mockProxy) {
        uint256 pid = mc.startProcess("createMockProxy", param(bundle));
    ...
    function createMockProxy(MCDevKit storage mc, bytes memory initData) internal returns (Proxy storage mockProxy) {
        uint256 pid = mc.startProcess("createMockProxy", param(initData));
    ...
    function createMockProxy(MCDevKit storage mc) internal returns (Proxy storage mockProxy) {
        uint256 pid = mc.startProcess("createMockProxy");
    ...
    {
        uint256 pid = mc.startProcess("createMockDictionary", param(bundle, owner));
        Validator.MUST_Completed(bundle);
        Validator.SHOULD_OwnerIsNotZeroAddress(owner);
        Dictionary memory _mockDictionary =
            DictionaryLib.createMock(bundle, owner).assignName(mc.dictionary.genUniqueMockName(bundle.name));
    ...
    function createMockDictionary(MCDevKit storage mc) internal returns (Dictionary storage mockDictionary) {
        uint256 pid = mc.startProcess("createMockDictionary");
    ...
    function createMockDictionary(MCDevKit storage mc, Bundle storage bundle)
        ...
        returns (Dictionary storage mockDictionary)
    {
        uint256 pid = mc.startProcess("createMockDictionary", param(bundle));
    ...
    function createMockDictionary(MCDevKit storage mc, address owner)
        ...
        returns (Dictionary storage mockDictionary)
    {
        uint256 pid = mc.startProcess("createMockDictionary", param(owner));
