...
library Formatter {
    ...
    function toString(Function memory func) internal pure returns (string memory message) {
        return message.append("Impl: ").append(func.implementation).comma().append("Selector: ").append(func.selector)
            .comma().append("Name: ").append(func.name);
    }
    ...
    function toString(Bundle storage bundle) internal view returns (string memory message) {
        ...
        for (uint256 i; i < _funcs.length; ++i) {
    ...
    function toStart(Process memory process, uint256 pid) internal pure returns (string memory) {
        ...
        string memory nest;
        for (uint256 i; i <= process.nest; ++i) {
            nest = string.concat(nest, "> ");
        }
        return header.append(nest.dim()).append(process.libName.dot().append(process.funcName)).append(
            process.params.parens().dim().italic()
        );
    }
