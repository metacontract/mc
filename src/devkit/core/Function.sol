...
struct Function {
    /// @dev TODO Function may be different depending on the op version.
    string name;
    bytes4 selector;
...
library FunctionLib {
    ...
    function assignName(Function storage func, string memory name) internal returns (Function storage) {
        uint256 pid = func.startProcess("assignName", param(name));
        func.startBuilding();
        func.name = name;
        func.finishBuilding();
    ...
    function assignSelector(Function storage func, bytes4 selector) internal returns (Function storage) {
        uint256 pid = func.startProcess("assignSelector", param(selector));
        func.startBuilding();
    ...
    function assignImplementation(Function storage func, address implementation) internal returns (Function storage) {
        uint256 pid = func.startProcess("assignImplementation", param(implementation));
        func.startBuilding();
    ...
    {
        uint256 pid = func.startProcess("assign", param(name, selector, implementation));
    ...
    function fetch(Function storage func, string memory envKey) internal returns (Function storage) {
        uint256 pid = func.startProcess("fetch", param(envKey));
